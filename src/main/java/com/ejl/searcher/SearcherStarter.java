...
import com.ejl.utils.clazz.ClassScanner;
...
public class SearcherStarter {
	 ...
	 * @param jarName 可检索 Bean 所在的 jar 名称
	 * @param packageName 可检索 Bean 所在的 package，可检索 Bean 是被 @SearchBean 注解的 Bean
	 * @return true if start successfully, else return false
	 * */
	...
	private boolean startWithBeanClassList(List<Class<?>> beanClassList) {
		...
		for (Class<?> beanClass: beanClassList) {
			...
			if (searchBean != null) {
				SearchBeanMap searchBeanMap = new SearchBeanMap(searchBean.tables(), searchBean.joinCond(), searchBean.distinct());
				for (Field field : beanClass.getDeclaredFields()) {
					...
					if (dbField != null) {
						...
						try {
							...
							throw new RuntimeException("【" + beanClass.getName() + "：" + fieldName + "】被注解的属性必须要有正确的set方法！");
						}
