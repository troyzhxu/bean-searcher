...
::: warning 注意
自 `v4.5` 版本起，默认支持 `JDK17 +`，如需兼容 `JDK 8 ~ 16`，可以使用带 `.jdk8` 后缀的兼容版本，例如：`v4.8.8.jdk8`。
:::

* Bean Searcher
  - 修复 `MapUtils.flat()` 配合多值参数使用时，`ARRAY_KEYS` 的 UUID 末尾段若为纯数字，会导致 `Integer.parseInt` 抛出 `NumberFormatException` 的问题（since v4.8.8）
  ...
  - 优化异常处理机制：当遇到不能处理的未知 SQL 语法时，抛出异常告诉用户哪段 SQL 不能解析（since v4.8.1）
  - 兼容 PgSQL 的 `||` 字符串连接符（since v4.8.1）
  - 嵌入参数结束标志支持方括号检测（since v4.8.3）
  - 修复 `SqlServerDialect` 在用户未指定排序字段时的分页查询的生成的 SQL 不符合 SQL Server 语法要求的问题（since v4.8.3）
...
::: warning 注意
自 `v4.5` 版本起，默认支持 `JDK17 +`，如需兼容 `JDK 8 ~ 16`，可以使用带 `.jdk8` 后缀的兼容版本，例如：`v4.7.1.jdk8`。
:::

* Bean Searcher
  - 优化 `ParamAware`、`ResultFilter` 与 `SqlInterceptor` 的 `paraMap` 参数值：使用经过 `ParamFilter` 处理后的非空值（since v4.7.1）
* Bean Searcher Exporter
  - 组件 `BeanExporter` 新增支持 `Function<List<T>, List<T>> mapper` 参数的系列导出方法，可在写入导出文件之前，对查询出的整体数据做二次加工：
...
::: warning 注意
自 `v4.5` 版本起，默认支持 `JDK17 +`，如需兼容 `JDK 8 ~ 16`，可以使用带 `.jdk8` 后缀的兼容版本，例如：`v4.6.0.jdk8`。
:::
...
::: warning 注意
自 `v4.5` 版本起，默认支持 `JDK17 +`，如需兼容 `JDK 8 ~ 16`，可以使用带 `.jdk8` 后缀的兼容版本，例如：`v4.5.2.jdk8`。
:::

* Bean Searcher
  - 注解 `@SearchBean` 新增 `maxSize` 与 `maxOffset` 属性，可以为单个检索类单独设置风控值，覆盖全局配置
  ...
  - 优化 `DefaultSqlExecutor`：如果 SQL 执行报错，则 SQL 日志级别从 `DEBUG` 提升为 `ERROR`（since v4.5.1）
  - 优化 `DefaultSqlExecutor`：如果 JDBC 在 `prepareStatement` 阶段报错，也打印出报错的 SQL（since v4.5.1）
  - 优化 `EnumFieldConvertor`：以支持 `short` 与 `byte` 向枚举转换。此前只支持 `String` 与 `int` 类型。（since v4.5.2）
...
* Bean Searcher
  ...
  - 优化 `EnumFieldConvertor`：以支持 `short` 与 `byte` 向枚举转换。此前只支持 `String` 与 `int` 类型。（since v4.4.3）
  - 优化 `DefaultSqlExecutor`：如果 SQL 执行报错，则 SQL 日志级别从 `DEBUG` 提升为 `ERROR`（since v4.4.3）
  - 优化 `DefaultSqlExecutor`：如果 JDBC 在 `prepareStatement` 阶段报错，也打印出报错的 SQL（since v4.4.3）
