...
// 如果希望使用字段标签（字典翻译）的功能
implementation 'cn.zhxu:bean-searcher-label:4.6.0.alpha'
// 如果希望使用数据导出的功能（可作为 EasyExcel 的替代方案） -->
// 非常轻量，不依赖 POI, 用法更简单
implementation 'cn.zhxu:bean-searcher-exporter:4.6.0.alpha'
```
```groovy [Grails 7.x]
// 直接使用此依赖，即可使用 Bean Searcher 的数据检索功能
implementation 'cn.zhxu:bean-searcher-boot-starter:4.6.0.alpha'
...
// 直接使用此依赖，即可使用 Bean Searcher 的数据检索功能
implementation 'cn.zhxu:bean-searcher-boot-starter:4.6.0.alpha.jdk8'
// 如果希望使用字段标签（字典翻译）的功能
implementation 'cn.zhxu:bean-searcher-label:4.6.0.alpha.jdk8'
// 如果希望使用数据导出的功能（可作为 EasyExcel 的替代方案） -->
// 非常轻量，不依赖 POI, 用法更简单
implementation 'cn.zhxu:bean-searcher-exporter:4.6.0.alpha.jdk8'
```
...
```groovy [Solon]
// Solon 项目直接使用此依赖（功能同 bean-searcher-boot-starter）
implementation 'cn.zhxu:bean-searcher-solon-plugin:4.6.0.alpha.jdk8'
// 如果希望使用字段标签（字典翻译）的功能
implementation 'cn.zhxu:bean-searcher-label:4.6.0.alpha.jdk8'
// 如果希望使用数据导出的功能（可作为 EasyExcel 的替代方案） -->
// 非常轻量，不依赖 POI, 用法更简单
...
```groovy [Others]
// Bean Searcher 核心依赖，任何框架都可使用该依赖，需要手动配置
implementation 'cn.zhxu:bean-searcher:4.6.0.alpha.jdk8'
// 如果希望使用字段标签（字典翻译）的功能，需要手动配置
implementation 'cn.zhxu:bean-searcher-label:4.6.0.alpha.jdk8'
// 如果希望使用数据导出的功能（可作为 EasyExcel 的替代方案），需要手动配置 -->
// 非常轻量，不依赖 POI, 用法更简单
...
<dependency>
    <groupId>cn.zhxu</groupId>
    <artifactId>bean-searcher-boot-starter</artifactId>
    <version>4.6.0.alpha.jdk8</version>
</dependency>
<!-- 如果希望使用字段标签（字典翻译）的功能 -->
<dependency>
    <groupId>cn.zhxu</groupId>
